...
@compile_mode("script")
class CouplingClassifier(torch.nn.Module):
...
class GatedCouplingPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        for (interaction,
            ...
            preds_cls     = readouts_transformers_cls(x_cls)[0].squeeze(-1)      # [B, C]
            preds_readout = readouts_transformers_regress(x_reg)[0].squeeze(-1)  # [B, C]
...
class ScaleShiftGatedCouplingPredictor(GatedCouplingPredictor):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self.scale_shift_regress = LearnableScaleShift(
            num_heads = 1,
            learn_shift = True
        )
        self.scale_shift_class = LearnableScaleShift(
            num_heads = 1,
            learn_shift = False
        )
